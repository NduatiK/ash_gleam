    ...
    Enum.reduce_while(resource_entry.actions, :ok, fn action_entry, :ok ->
      cond do
        ...
        action = Ash.Resource.Info.action(resource_entry.resource, action_entry.action) ->
          if action.type in [:read, :create, :destroy, :action] or action.name == :get do
            {:cont, :ok}
          else
            {:halt,
             {:error,
              Spark.Error.DslError.exception(
                module: resource_entry.resource,
                message:
                  "FFI action #{inspect(action_entry.action)} must be a :read, :create, :destroy, :action, or :get action"
              )}}
          end
