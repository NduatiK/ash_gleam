defmodule AshGleam.FFITest do
  ...
  test "generated create ffi bridge creates a resource", %{bridge: _bridge} do
    builder = apply(:create_todo, :new, ["Created from ffi", false])

    assert {:ok, {:todo, id, "Created from ffi", false}} =
             apply(:create_todo, :run, [builder])
  ...
  test "generated get ffi bridge returns the requested resource", %{bridge: _bridge} do
    ...
    builder =
      todo.id
      |> then(&apply(:get_todo, :new, [&1]))
      |> then(
        &apply(:get_todo, :fields, [
          &1,
          [
            apply(:todo_item, :title_field, []),
            apply(:todo_item, :completed_field, [])
          ]
        ])
      )

    assert {:ok, {:todo, id, "Fetch me", false}} =
             apply(:get_todo, :run, [builder])
  ...
  test "generated list ffi bridge applies fields, filter, sort, and limit", %{bridge: _bridge} do
    ...
    builder =
      apply(:list_todos, :new, [])
      |> then(
        &apply(:list_todos, :fields, [
          &1,
          [
            apply(:todo_item, :title_field, []),
            apply(:todo_item, :completed_field, [])
      ...
      |> then(&apply(:list_todos, :filter, [&1, [apply(:todo_item, :completed_eq, [true])]]))
      |> then(&apply(:list_todos, :sort, [&1, [apply(:todo_item, :title_asc, [])]]))
      |> then(&apply(:list_todos, :limit, [&1, {:some, 1}]))

    assert {:ok, [{:todo, _id, "A task", true}]} =
             apply(:list_todos, :run, [builder])
  end
  ...
  test "generated list ffi bridge filters by title across multiple todos", %{bridge: _bridge} do
    ...
    builder =
      ...
      |> then(
        &apply(:list_todos, :filter, [&1, [apply(:todo_item, :title_eq, ["Filter target"])]])
      )
      |> then(&apply(:list_todos, :sort, [&1, [apply(:todo_item, :title_asc, [])]]))
      |> then(&apply(:list_todos, :limit, [&1, :none]))
